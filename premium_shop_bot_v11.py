...
ចំណាំ (v13): Mini App ឥឡូវជាច្រកចូលតែមួយគត់ — /start បង្ហាញប៊ូតុង web_app តែមួយ (ចាស់ៗ
  ...
  (ដូច /addstock command) តាម background thread។ បន្ថែម /api/admin/broadcast endpoint។
  Bug fix សំខាន់: premium_text() (ប្រព័ន្ធ 🎭 Setup Emoji) អាច corrupt HTML ពេល glyph
  overlap គ្នា ធ្វើឲ្យសារទាំងមូល (រួមទាំង /start) បដិសេធដោយ Telegram ("can't parse
  entities") — កែជា placeholder-token replace + retry ជាអត្ថបទធម្មតាបើនៅតែបរាជ័យ។
ចំណាំ (v12): បានលុបជម្រើសទូទាត់តាម ABA (khpay.site) ចេញ — Deposit ទាំងអស់ប្រើ Bakong KHQR (CamRapidPay) ជាតែមួយ
ចំណាំ (v11): បន្ថែម 🤖 ជាវ Bot ផ្ទាល់ខ្លួន (inline button ក្នុង 🛒 ទិញ Account, admin លុបមិនបានទេ
  ព្រោះមិនស្ថិតក្នុង products.json)។ Flow: user ផ្ញើ Bot Token (@BotFather) → បើមាន Bakong ID
  ខ្លួនឯង ផ្ញើ Bakong ID (គ្មានគិតថ្លៃ) → បើគ្មាន សូមបញ្ចូលចំនួនថ្ងៃចង់ជួល ($0.15/ថ្ងៃ, កាត់ពី Wallet)
  → admin ទទួលសារ ហើយចុច "✅ បានដាក់ Bot ជូនរួចរាល់" បន្ទាប់ពីដាក់ដំណើរការដោយដៃ (/subs មើលបញ្ជីរង់ចាំ,
  /activatesub user_id ជា fallback)។
"""
...
# URL របស់ Mini App (miniapp.html ដែល serve នៅ route "/") — ប្រើសម្រាប់ប៊ូតុង
# web_app តែមួយគត់ក្នុង /start ។ អាចកំណត់ដោយដៃតាម Env Var MINIAPP_URL បើចង់
MINIAPP_URL = os.environ.get(
    "MINIAPP_URL",
    f"{PUBLIC_BASE_URL.rstrip('/')}/" if PUBLIC_BASE_URL else "",
)
STORE_NAME = os.environ.get("STORE_NAME", "Kairozen Store")  # ឈ្មោះហាង — hardcode ជា default តែអាច override តាម env (ចាំបាច់សម្រាប់ bot subscriber clone)
...
if _NOTIFY_CHAT_IDS_ENV is not None:
    # បើកំណត់ env var (សូម្បីជា string ទទេ) → override list hardcode ខាងក្រោម
    # ចាំបាច់សម្រាប់ bot subscriber clone ដើម្បីកុំឲ្យវាផ្ញើសារចូល channel របស់ហាងមេ
    NOTIFY_CHAT_IDS = [
...
def premium_text(text):
    ...
    ចំណាំសំខាន់ (bug fix): glyph ខ្លះជា substring របស់ glyph ផ្សេងទៀត (ឧ. emoji
    មាន/គ្មាន variation-selector U+FE0F ដូចគ្នា) — បើ replace ជាបន្តបន្ទាប់ត្រង់ៗ
    ដូចមុន វា replace ចូលទៅក្នុង tag ដែលទើបតែបញ្ចូលរួច ធ្វើឲ្យ tag ខូច (nested/broken
    <tg-emoji>) ហើយ Telegram បដិសេធសារទាំងមូល ("can't parse entities"/"Empty
    attribute name") — ធ្វើឲ្យសារមិនចេញសោះ (ឧ. /start ឬ broadcast បរាជ័យទាំងអស់)។
    ការពារដោយប្រើ placeholder token ជាមុនសិន រួច replace ត្រឡប់ជា HTML នៅចុងក្រោយ
    តែម្តង ដើម្បីកុំឲ្យវគ្គបន្ទាប់ replace ត្រូវលើ tag ដែលបានបញ្ចូលរួច។"""
    if not text:
        return text
    m = get_emoji_map()
    if not m:
        return text
    # glyph វែងបំផុតមុន — កុំឲ្យ glyph ខ្លីត្រូវនឹង substring របស់ glyph វែងជាមុន
...
#   {
#     "status": "collecting_bakong_choice" | "waiting_admin_deploy" | "active",
#     "bot_token": "<Bot Token ពី @BotFather ដែល user ដាក់ជូន>",
#     "bakong_id": "<Bakong ID ផ្ទាល់ខ្លួនរបស់ user>" ឬ None (បើជួលគិតជាថ្ងៃ),
#     "qr_photo_file_id": "<Telegram file_id នៃរូបភាព QR ផ្ទាល់ខ្លួន>" ឬ None (បើមាន Bakong ID),
#     "rental_days": <int ចំនួនថ្ងៃដែលជួល> ឬ None (បើមាន Bakong ID ខ្លួនឯង),
#     "amount_paid": <float ចំនួនលុយបានទូទាត់សម្រាប់ជួល> ឬ None,
#     "requested_at": "<timestamp text>",
#     "activated_at": "<timestamp text>" ឬ None (កំណត់ពេល admin ចុច 'បានដាក់ Bot ជូនរួចរាល់'),
#   }
def load_subs():
...
# ------------------------------------------------------------------
# ពេល admin ចុច "✅ បានដាក់ Bot ជូនរួចរាល់" bot នេះនឹង spawn process ថ្មីមួយ ជា
# copy ដដែលនៃ script នេះ (ប្រើ __file__ ដដែល) តែផ្លាស់ប្តូរ Environment Variables ដូចជា
# BOT_TOKEN / ADMIN_ID / DATA_DIR ជាដើម ដើម្បីឲ្យវាដំណើរការជា bot ដាច់ដោយឡែកសម្រាប់ subscriber។
# process ចាំបាច់ត្រូវជា detached (start_new_session=True) ដើម្បីកុំឲ្យ Render/OS បិទវា
# ពេល process មេ (bot នេះ) restart ។
...
def stop_subscriber_bot(uid):
    """បញ្ឈប់ process bot របស់ subscriber (បើកំពុងរត់) — ប្រើមុន redeploy ថ្មី ឬពេលចង់បិទ"""
    rec = get_sub(uid) or {}
...
def deploy_subscriber_bot(uid, rec=None):
    ...
    ដោយប្រើ Bot Token + (បើមាន) CAMRAPIDPAY_API_KEY ផ្ទាល់ខ្លួន។ Return (True, pid) ឬ (False, error_text)"""
    rec = rec or get_sub(uid) or {}
    bot_token = rec.get("bot_token")
    if not bot_token:
        return False, "គ្មាន Bot Token ក្នុង record នេះទេ"
    stop_subscriber_bot(uid)  # បញ្ឈប់ process ចាស់ (បើមាន) មុន redeploy
    ...
    try:
        ...
        proc = subprocess.Popen(
            ...
            start_new_session=True,  # detach ពី process មេ ដើម្បីកុំឲ្យស្លាប់តាម
...
def cmd_start(message):
    ...
    text = (
        ...
        f"👉 ចុចប៊ូតុង <b>🛍 បើក {STORE_NAME}</b> ខាងក្រោមនេះ ដើម្បីចូលទៅកាន់ហាង — "
        f"ទិញ account, បញ្ចូលលុយ Wallet, មើលការកម្មង់ និងណែនាំមិត្ត ទាំងអស់នៅកន្លែងតែមួយ។\n\n"
...
def subscribe_token_step(message, from_user):
    """ទទួល Bot Token ពី user → រក្សាទុកបណ្តោះអាសន្ន ហើយសួរឈ្មោះហាងដែលចង់ដាក់"""
    uid = from_user.id
    chat_id = message.chat.id
...
def subscribe_storename_step(message, from_user):
    """ទទួលឈ្មោះហាងពី user → រក្សាទុក ហើយបន្តសួរថាតើមាន Bakong ID ខ្លួនឯងឬអត់"""
    uid = from_user.id
    chat_id = message.chat.id
...
def subscribe_qr_photo_step(message, from_user):
    """ទទួលរូបភាព QR ផ្ទាល់ខ្លួន (ករណីគ្មាន Bakong ID) → រក្សាទុក file_id រួចបន្តទៅសួរចំនួនថ្ងៃដែលចង់ជួល"""
    uid = from_user.id
    chat_id = message.chat.id
...
def admin_subdeploy_apikey_step(message, target_uid):
    """ទទួល CAMRAPIDPAY_API_KEY ពី admin (សម្រាប់ subscriber ដែលមាន Bakong ID ខ្លួនឯង)
    រួច spawn subprocess ស្វ័យប្រវត្តិ"""
...
def callback_router(call):
    ...
    if data == "menu_shop":
        ...
        if rec.get("bakong_id") and not rec.get("camrapidpay_api_key"):
            # ត្រូវការ CAMRAPIDPAY_API_KEY ដែល admin បង្កើតឲ្យ bakong_id នេះនៅ CamRapidPay
            # dashboard ជាមុនសិន — ជំហាននេះនៅតែធ្វើដោយដៃ ព្រោះជា step ក្រៅ Telegram
            bot.answer_callback_query(call.id)
            msg = bot.send_message(
...
def cmd_activatesub(message):
    """Admin fallback ដៃ បើប៊ូតុង '✅ បានដាក់ Bot ជូនរួចរាល់' បាត់ ឬចង់ deploy ដោយផ្ទាល់តាម command
    ទំរង់: /activatesub user_id  ឬ  /activatesub user_id|camrapidpay_api_key (បើមាន Bakong ID ខ្លួនឯង)"""
    if not is_admin(message.from_user.id):
        return
    try:
...
def cmd_stopsub(message):
    """Admin បញ្ឈប់ process bot របស់ subscriber មួយ (ដោយមិនលុប record ចោល — deploy ម្តងទៀតបានតាមក្រោយ)"""
    if not is_admin(message.from_user.id):
        return
    try:
...
def cmd_subs(message):
    """Admin មើលបញ្ជីសំណើជាវ Bot ដែលកំពុងរង់ចាំ + បញ្ជី Bot ដែល active (ដោយបង្ហាញថាតើ process
    កំពុងរត់ពិតឬអត់ តាម PID)"""
    if not is_admin(message.from_user.id):
        return
    subs = load_subs()
...
def start_keep_alive():
    ...
    def api_admin_stock_add_route():
        ...
        added = len(lines)
        # reset ការជូនដំណឹង "ជិតអស់ស្តុក" ព្រោះទើបតែបញ្ចូល stock ថ្មី (ដូច process_addstock)
        # — ធ្វើក្នុង _lock ហើយ reload products.json ស្រស់ៗមុននឹង save ដើម្បីកុំឲ្យសរសេរជាន់
        # លើការកែប្រែផ្សេងទៀត (ឧ. admin កែ product មួយផ្សេងទៀតក្នុងពេលដំណាលគ្នា) → ការពារ
        # ទិន្នន័យបាត់ (users.json ខ្លួនឯងមិនត្រូវប៉ះពាល់ទាល់តែសោះ — broadcast អានតែម្នាក់ៗ
        # ពី load_users() មិនសរសេរអ្វីទៅ users.json ទេ)
        with _lock:
            fresh_products = load_products()
            if key in fresh_products and fresh_products[key].get("low_stock_alerted"):
