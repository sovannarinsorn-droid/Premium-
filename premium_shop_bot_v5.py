...
def qty_pick_kb(key, qty, max_qty, unit_price):
    ...
    kb.add(pbtn(f"✅ បន្ត — សរុប ${unit_price * qty:.2f}", callback_data=f"qtyok_{key}_{qty}", style="success"))
...
def buy_confirm_kb(key, unit_price, qty):
    """ជ្រើសរើសវិធីទូទាត់: ពី Wallet ឬ ទូទាត់ផ្ទាល់ KHQR (មិនចាំបាច់ដាក់លុយចូល Wallet)"""
    total = unit_price * qty
    kb = types.InlineKeyboardMarkup(row_width=1)
    kb.add(pbtn(f"💰 ទូទាត់ពី Wallet (${total:.2f})", callback_data=f"buywallet_{key}_{qty}"))
    kb.add(pbtn("⚡ ទូទាត់ផ្ទាល់ KHQR (មិនចាំបាច់ Wallet)", callback_data=f"buyqr_{key}_{qty}"))
    kb.add(pbtn("🔙 ត្រឡប់ក្រោយ", callback_data=f"backqty_{key}_{qty}"))
    return kb
...
def deposit_amount_kb():
    ...
    kb.add(pbtn("✏️ បញ្ចូលចំនួនផ្សេង", callback_data="dep_custom"))
    kb.add(pbtn("🔙 ត្រឡប់ក្រោយ", callback_data="back_main"))
    return kb
...
def callback_router(call):
    ...
    if data == "menu_shop":
        ...
        show_buy_options(call, key, int(qty_s))

    elif data.startswith("backqty_"):
        key, qty_s = data[len("backqty_"):].rsplit("_", 1)
        show_qty_picker(call, key, int(qty_s))

    elif data.startswith("buywallet_"):
        key, qty_s = data[len("buywallet_"):].rsplit("_", 1)
        handle_buy_wallet(call, key, int(qty_s))

    elif data.startswith("buyqr_"):
        key, qty_s = data[len("buyqr_"):].rsplit("_", 1)
        handle_buy_qr(call, key, int(qty_s))
...
def show_buy_options(call, product_key, qty=1):
    chat_id = call.message.chat.id
    products = load_products()
    if product_key not in products:
        bot.answer_callback_query(call.id, "❌ Product មិនត្រឹមត្រូវ", show_alert=True)
    ...
    bot.edit_message_text(
        ...
        chat_id, call.message.message_id, reply_markup=buy_confirm_kb(product_key, p["price"], qty),
    )
...
def handle_buy_qr(call, product_key, qty=1):
    ...
    resp = camrapid_create(total_price, ref)
    if not resp:
        bot.answer_callback_query(
            call.id,
    ...
    img_buf = build_qr_image(
        qr_string, amount=total_price, ref=ref_disp,
        label=f"{product['name']} × {qty}", subtitle=f"{STORE_NAME} · ទិញផ្ទាល់",
    ) if qr_string else None

    caption = (
        f"{icon} <b>{product['name']}</b> × {qty}\n💵 <b>${total_price:.2f}</b>\n🔖 <code>{ref_disp}</code>\n\n"
...
def poll_purchase(uid, chat_id, product_key, product_name, price, reference, qty=1, user_label=None, max_minutes=5):
    ...
    while time.time() < deadline:
        if camrapid_check(reference):
            items = pop_stock_items(product_key, qty)
            if len(items) < qty:
                push_stock_items(product_key, items)  # ដាក់ត្រឡប់វិញ បើយកមិនគ្រប់
                # ស្តុកអស់ត្រឹមតែមុនពេលទូទាត់ចប់បន្តិច — admin ត្រូវសង​ប្រាក់ ឬបន្ថែម stock ដោយដៃ
                try:
                    bot.send_message(
                        chat_id,
                        f"⚠️ ទូទាត់ជោគជ័យសម្រាប់ <b>{product_name}</b> × {qty} (${price:.2f}) ប៉ុន្តែស្តុកអស់ភ្លាមៗ។\n"
                ...
                if ADMIN_ID:
                    try:
                        bot.send_message(
                            ADMIN_ID,
                            f"🚨 URGENT: user {uid} ទូទាត់ {product_name} × {qty} (${price:.2f}) ជោគជ័យ ប៉ុន្តែស្តុកអស់!\n"
            ...
            users[uid_str]["orders"] = users[uid_str].get("orders", 0) + qty
            save_users(users)
            ...
            try:
                bot.send_message(
                    ...
                    f"🛍️ Product: <b>{product_name}</b> × {qty}\n"
                    f"💵 សរុប: ${price:.2f}\n\n"
                    f"🔑 <b>Account របស់អ្នក:</b>\n{accounts_text}",
                )
            except Exception:
            ...
            if ADMIN_ID:
                try:
                    bot.send_message(
                        ADMIN_ID,
                        f"🔔 លក់ថ្មី (KHQR ផ្ទាល់): {product_name} × {qty} (${price:.2f}) ដល់ user {uid}\n"
                        f"ស្តុកនៅសល់: {stock_count(product_key)}",
                    )
                    if stock_count(product_key) <= 2:
